...
void Darts_501::takeTurn() {
    ...
    do {
        if (score > 109)												
            state_of_player = fast;
        else if (score > 50)
            state_of_player = slow;
        else if (score == 50)
            state_of_player = earlyfinish;
        else if (score > 40)
            state_of_player = focus;
        else
            state_of_player = out;
        
        switch (state_of_player) {                                                  // Using a switch statement for my enum cases in order to make the program function how i want
        ...
        darts++;														//signal dart has been used
    } while ((score > 1) && (darts < 3));								// loop until all darts are used AND the score surpasses 1
    if (score == 1 || score < 0)									
        score = temp;
}
